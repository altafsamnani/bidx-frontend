...
class BidxCommon {
	...
	*/
	...
	function forceWordpressLogin( $subDomain ) {
		...
		if ( $sessionData != null 
				&& property_exists( $sessionData, 'authenticated' ) 
	    	...
	    	Logger :: getLogger('Login') -> trace('Forcing Login as '. $userName);
			...
			if ( $user_id = username_exists( $userName ) ) {   
		        ...
		        Logger :: getLogger("Login") -> trace("Userdata ". $userData);
				...
				// set the cookie to secure if needed (check environment --> default secure atm)
				...
				Logger :: getLogger( 'Login' ) -> trace( 'User '. $userName . ' does not exist' );
				//user unknown : redirect
			}
		} 
    	return;
	}
	...
	/**
	 * Clears out the bidX related cookies
	 */
